/**
 * A leg the platform dialled on an INFO it accepted as a transfer order (RFC
 * 6086) is a transferee, not a fork: the join reading moves it off the hunt,
 * the actor stays the callee it is on the wire, and `joined_by` names the INFO
 * that asked for it.
 */
...
/**
 * A answered by B; B sends an INFO; the platform 200s, dials C, and once C
 * answers releases B. The caller ends the call.
 */
const infoTransferFlows = (): Flows.FlowsDoc =>
  doc(
    [
      ...
      leg(CALLEE_CALL_ID, oneHop(sut, callee), [
        ...
        request({ callId: CALLEE_CALL_ID, seq: 1, method: "INFO", src: callee, dst: sut, ts_ms: 3_000, fromUri: CALLEE_URI, fromTag: "callee-tag", ruri: CALLER_URI, toTag: `from-${CALLEE_CALL_ID}`, headers: ["Content-Type: application/vnd.example.transfer"] }),
        response({ callId: CALLEE_CALL_ID, seq: 1, status: 200, reason: "OK", cseqMethod: "INFO", src: sut, dst: callee, ts_ms: 3_010, fromUri: CALLEE_URI, fromTag: "callee-tag", toUri: CALLER_URI, toTag: `from-${CALLEE_CALL_ID}` }),
...
/** The reading a deployment gives: the leg the platform dialled toward `other` joined on an INFO. */
const transfereeJoined = (_flows: Flows.FlowsDoc, actors: ReadonlyArray<ActorObs>): JoinsReading =>
  new Map([[actors.findIndex((a) => a.peer === other), { kind: "info" as const, evidence: ["stub"] }]])

const cutOf = (joins: typeof transfereeJoined) => {
  const flows = infoTransferFlows()
...
describe("a leg joined on an INFO", () => {
  ...
  it("is joined by the INFO step, on whichever leg sent it", () => {
    const step = flow.steps.find((s) => s.id === attempt.joined_by!.step)!
    expect(attempt.joined_by!.kind).toBe("info")
    expect(step.msg.method).toBe("INFO")
