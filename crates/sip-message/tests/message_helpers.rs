...
fn header_param_value_reads_icid_from_leading_position() {
    ...
    let pcv = format!("icid-value={ICID};icid-generated-at=198.51.100.63");
...
fn header_param_value_is_order_independent_and_ignores_mutating_siblings() {
    ...
    let a = format!("icid-value={ICID};icid-generated-at=198.51.100.63;orig-ioi=orange.fr");
    let b = format!("orig-ioi=bar.example.com;term-ioi=x;icid-value={ICID}");
...
fn user_identity_matches_tel_and_sip_forms_across_hosts() {
    ...
    assert!(same_user_identity(
        "tel:+33000900011",
        "sip:+33000900011@ims.mnc001.mcc208.3gppnetwork.org;user=phone",
    ));
    assert!(same_user_identity("<tel:+33000900011>", "sip:+33000900011@anything:5070"));
}

#[test]
fn user_identity_drops_userinfo_params_and_uri_params() {
    // Real capture form: `verstat` rides the USER part, before the `@`.
    let a = "sip:+33000900012;verstat=TN-Validation-Passed@bar.example.com:5060;user=phone";
    let b = "sip:+33000900012;verstat=No-TN-Validation@baz.example.com;user=phone";
    assert_eq!(uri_user_identity(a).as_deref(), Some("+33000900012"));
...
fn user_identity_negative_cases() {
    assert!(!same_user_identity("tel:+33000900011", "sip:+33772589501@host"));
