...
async fn real_http_admit_reject_release_metrics() {
    ...
    let r1 = net.request(dst, HttpRequest::post("/v1/admit", admit_body.clone())).await.unwrap();
    ...
    let r2 = net.request(dst, HttpRequest::post("/v1/admit", admit_body.clone())).await.unwrap();
    ...
    let rel = serde_json::to_vec(&ReleaseRequest {
        entries: vec![Hold { id: "trunk-A".into(), window }],
    })
    .unwrap();
    let rr = net.request(dst, HttpRequest::post("/v1/release", rel)).await.unwrap();
    assert_eq!(rr.status, 200);

    let r3 = net.request(dst, HttpRequest::post("/v1/admit", admit_body)).await.unwrap();
