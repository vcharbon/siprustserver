...
//! [`PeerScope`]. Rendered as `sip_proxy_peer_failures_total{peer,scope,kind}`.
//!
...
//!   - **internal peers** (the cluster: any destination that resolves to a known
//!     worker via the registry) are a naturally-tiny set and are **pinned** —
//!     always tracked, never evicted.
...
pub enum PeerFailureKind {
    ...
    /// exceeds what this path carries even fragmented (ADR-0026). Split out
...
mod tests {
    ...
    fn prometheus_text_renders_labels_and_conserves_total() {
        ...
        let text = pf.prometheus_text("sip_proxy_peer_failures_total");
        ...
        let sum: u64 = text
            .lines()
            .filter(|l| l.starts_with("sip_proxy_peer_failures_total{"))
