...
impl ProxyCore {
    ...
    pub(super) async fn route_request(&self, msg: &SipMessage, src: SocketAddr) -> RouteOutcome {
        ...
        // (upstreamneed/014, 486-then-reroute). Gating on the marker (rather
    ...
    async fn reply(&self, req: &SipRequest, src: SocketAddr, status: u16, reason: &str, extra: &[SipHeader]) {
        ...
        // transaction it ever created (the upstreamneed-014 class). No
...
mod ack_absorption_tests {
    ...
    //! the absence of a Route header (upstreamneed/014). A b-leg INVITE behind
