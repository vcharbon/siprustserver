...
impl TimerType {
    ...
    /// cancel site must mint from (drift = a cancel that silently misses; see
    /// the CLAUDE.md timer-aliasing hazard): `{self:?}` for a call-level timer,
    /// `{self:?}:{leg_id}` for a per-leg one.
...
pub struct Call {
    ...
    /// last applied `Route` (upstreamneed-009): when a subscribed event fires
    ...
    /// (a `Route`-shaped `call_release` decision, upstreamneed-009 piece 3):
...
/// `Route` decision (upstreamneed-009). When a subscribed event fires, the core
...
/// `release-reroute` treatment (upstreamneed-009 piece 3), gating its rules the
