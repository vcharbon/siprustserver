...
/// call-wide `seq`. `Call::message_seq` is the `seq` of the last message
/// recorded on any leg, so the two rings of a call interleave by it. A cap of
/// `0` records nothing and moves nothing; an unknown leg is left alone.
