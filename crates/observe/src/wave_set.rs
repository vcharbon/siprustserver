...
//! no runtime (a synchronous unit test) simply loses the periodic line, never
//! the rising or the recorded totals.
//!
//! [`WaveSet::is_active`] is a single relaxed atomic load: a hot path that only
//! needs to *close* an episode (an admit after a shed, a success after an
//! outage) pays one predicted branch while everything is quiet.
//!
...
pub struct WaveSet {
    ...
    /// Number of open episodes — the cheap "anything happening?" flag.
...
impl WaveSet {
    ...
    /// Whether any episode is open — one relaxed load, for hot paths that only
    /// need to act when something is already burning.
    pub fn is_active(&self) -> bool {
        self.open.load(Ordering::Relaxed) > 0
    }

    /// Record one event of `counter` against `key`, emitting whatever line that
    /// makes due. Ignored once [`MAX_KEYS`] episodes are open.
    pub fn record(self: &Arc<Self>, key: &str, counter: &'static str, n: u64) {
        ...
        let (report, generation) = {
            ...
            (w.record(counter, n), w.generation())
        };
    ...
    /// End `key`'s episode now (a recovery, a completed sweep) and emit its
    /// falling edge. No-op when nothing is open for `key`.
    pub fn close(&self, key: &str) {
        ...
        let report = wave.lock().unwrap().close();
        if let Some(report) = report {
            self.finish(key, &report);
        }
    }

    /// End every open episode (process drain / test teardown).
    pub fn close_all(&self) {
        if !self.is_active() {
            return;
        }
        let entries: Vec<(String, Arc<Mutex<Wave>>)> = self
            .waves
            .lock()
            .unwrap()
            .iter()
    ...
    /// Emit a falling edge and free the key's slot.
    fn finish(&self, key: &str, report: &WaveReport) {
...
mod tests {
    ...
    async fn keys_are_independent_episodes() {
        ...
        assert_eq!(lines[2], ("w0".into(), Edge::Falling, 1));
        assert!(set.is_active(), "w1's episode is still open");
        set.close_all();
        assert!(!set.is_active());
    }

    /// The key bound holds against an unbounded key source, and a closed key
    /// frees its slot.
    #[tokio::test(start_paused = true)]
