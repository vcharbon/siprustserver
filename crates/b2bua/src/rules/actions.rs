...
impl<'a> ActionExecutor<'a> {
    ...
    fn confirm_dialog(&self, call: &mut Call, ctx: &RuleContext, leg_id: &str) {
        ...
        //    `Terminating`, its CDR never flushes; upstreamneed-027). The a-leg is
    ...
    /// effects, → terminated too (upstreamneed-028: the answered leg is resolved,
                ...
                LegState::Trying | LegState::Early => {
                    if is_a {
                        ...
                        *call = set_bye_disposition(call.clone(), &id, ByeDisposition::None);
                        // Parity with the b-leg arm (upstreamneed-028): when that
    ...
    fn send_request_to_leg(
        ...
        *call = bump_local_cseq(call.clone(), leg_id, &t_id, 1);
        ...
        // upstreamneed/021). Body-owned headers are dropped: `body`/`content_type`
