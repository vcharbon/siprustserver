...
pub async fn handle_initial_invite(
    ...
    // sampled call's story starts at its own INVITE. The engine force-enable
    // below is the only other entry, and it backfills what it missed.
    activate_intake(&mut call, &a_invite, now_ms);
    let request_json = trace::sampled(&call).then(|| json_body(&req));

    let response = decision.new_call(req).await;
    if let Ok(NewCallResponse::Route(route)) = &response {
        if route.trace {
            force_enable(&mut call, &a_invite, now_ms);
        }
    }
    record_new_call(&call, &a_invite, request_json, &response, now_ms);
...
// ── Per-call trace intake (ADR-0026) ────────────────────────────────────────

/// Run the trace admission chain for a brand-new call and stamp the correlation
/// triple on it. The `X-Trace-Sample` rate override is looked up ONLY when this
/// process honors the header AND exports at all — with no collector no rate can
/// change any outcome, so the header is not even read.
fn activate_intake(call: &mut Call, a_invite: &SipRequest, now_ms: i64) {
    let traces = trace::traces();
    let rate = intake_rate(&traces, a_invite);
    let Some(ids) = traces.activate(&call.call_ref, trace::registry::call_identity(call), rate, now_ms)
    else {
        return;
    };
    stamp(call, ids);
    record_intake(call, a_invite);
}
...
pub(crate) fn intake_rate(traces: &trace::CallTraces, a_invite: &SipRequest) -> Option<f64> {
...
/// **backfilled** — the INVITE it arrived on and the auto-100 the transaction
/// layer already sent are recorded at THEIR timestamps, not at activation time.
fn force_enable(call: &mut Call, a_invite: &SipRequest, now_ms: i64) {
    if trace::sampled(call) {
        return;
    }
    let traces = trace::traces();
    let Some(ids) =
        traces.activate(&call.call_ref, trace::registry::call_identity(call), Some(1.0), now_ms)
    else {
        return;
    };
    stamp(call, ids);
...
fn stamp(call: &mut Call, ids: crate::trace::registry::TraceIds) {
...
/// The two facts that precede any decision: the INVITE as it arrived, and the
/// auto-100 sip-txn sent for it (ADR-0022) — whose bytes this layer never held,
/// so it is recorded as a note rather than a wire body.
fn record_intake(call: &Call, a_invite: &SipRequest) {
    let wire = sip_message::serialize(&SipMessage::Request(a_invite.clone()));
    let src: SocketAddr = format!("{}:{}", call.a_leg.source.address, call.a_leg.source.port)
        .parse()
        .unwrap_or_else(|_| SocketAddr::from(([0, 0, 0, 0], 0)));
    trace::emit::sip_in(call, call.created_at, src, &wire);
    trace::emit::sip_out_note(call, call.created_at, "100 Trying (transaction layer)");
}

/// Record the `/call/new` round trip as a child span. On the force-enable path
/// the request body is rebuilt from the a-leg INVITE — it is derived from it, so
/// the rebuilt body IS the body that was sent — rather than buffered for every
/// unsampled call.
fn record_new_call(
    call: &Call,
    a_invite: &SipRequest,
    request_json: Option<Vec<u8>>,
    response: &Result<NewCallResponse, crate::decision::CallDecisionError>,
    at_ms: i64,
) {
    if !trace::sampled(call) {
        return;
    }
    let request = request_json.unwrap_or_else(|| json_body(&build_request(a_invite)));
    ...
    trace::emit::round_trip(call, "/call/new", at_ms, &request, at_ms, outcome, &body);
}
