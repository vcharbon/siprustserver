...
pub(super) async fn process(ctx: &Arc<RouterCtx>, event: CallEvent, res: Resolution) {
    ...
    let result = if res.initial_invite {
        ...
        // (`res.initial_invite`); re-INVITEs (To-tag present) and non-INVITE
        // in-dialog requests take the other branch and are never gated.
        //
        ...
        // → Completed with proper retransmission + ACK absorption) rather than
        // as a wire-raw datagram. It is still **stateless at the call layer**
        // — no `build_initial_call`/`create`, so no dialog, CDR, limiter hold,
        // or replicated state is ever born; we `return` before any of it.
        ...
        if !decision.admit {
            ...
            // Stateless shed, but this dispatch created a per-call queue (one
            // `bump_creation`; the retransmit guard above proves the call_ref is
            // brand-new) and took the per-call lock — and nothing will ever emit
            // `RemoveCall`. Release through the one teardown executor
            // (`ReleaseKind::Orphan`: no store mutation, so no spurious
            // reverse-propagated delete) so the worker exits and `removals`
            // balances `creations`. Drop our guard first.
            drop(_guard);
            release_call(ctx, &call_ref, ReleaseKind::Orphan).await;
            return;
        }
        ...
        let mut call = match ctx.state.hydrate_from_replica(&call_ref).await {
            Some((c, fresh, skew_offset_ms)) => {
                ...
                // node. `restore` past-due entries fire immediately (the
                // keepalive then re-arms itself on the next interval via the
                // `keepalive` rule); re-arming is idempotent — any subsequent
                // rule-emitted `ScheduleTimer` for the same id supersedes it via
                // the driver's epoch bump. Skipped for `fresh == false` (the
                // call was already resident and its timers are already live) to
                // avoid double-arm.
                if fresh {
            ...
            // arriving request. Refusing to look would 481 a healthy long-hold
            // call whose state lives RIGHT HERE. Materialise on demand, exactly
            // as the reactive straggler path does (timers restored, no smoothing
            // — one call), under the per-call guard we already hold; the bulk
            // sweep's own `materialize_if_absent` keeps the two passes
            // idempotent. NOT a takeover: this is our own call — no mark, no
            // self-release watch. A call never imported into `pri:{self}` (its
            // only copy is the peer's `bak:{self}`) still orphans — recovering
            // THAT population needs an on-demand pull from the peer (s11 CASE B,
            // open).
            None => match ctx.state.peek_reclaimable(&call_ref).await {
                ...
                None => {
                    ...
                    // `bump_creation`) and took the per-call lock, but resolved to
                    // NO live call — nothing will ever emit `RemoveCall`, and a
                    // per-call dispatch worker exits ONLY on poison, so the queue,
                    // its idle task, the unmatched creation, and the lock entry
                    // would all leak permanently (a mass-orphan failover turns
                    // that into an `active_calls`/`store_locks` ratchet that never
                    // drains). Release through the one teardown executor
                    // (`ReleaseKind::Orphan` — no store mutation, so no spurious
                    // reverse-propagated delete). Drop our guard first so the
                    // poisoned worker never contends on this call_ref's (now
                    // removed) lock.
                    drop(_guard);
                    release_call(ctx, &call_ref, ReleaseKind::Orphan).await;
                    return;
        ...
        if matches!(
            ...
            // EVERY in-dialog rule-chain event bumps the counter; if the bump
            // crosses `max_messages_per_call` and the handler did not itself
            // terminate the call, append a begin-termination so a runaway
            // dialog (re-INVITE/OPTIONS storm, glare loop, a peer that never
            // stops) is torn down instead of processing unbounded in-dialog
            // events forever — each of which allocates a txn (`set_txn`), a
            // working `Call` clone, and a store body. Initial-INVITE (the `if`
            // arm) and the async limiter-refresh do NOT count. The bump rides
            // the existing per-event flush — `message_count` adds no extra
            // replication traffic (it mutates with the CSeq/state the event
            // already changes). Order: bump + capture `cap_exceeded` BEFORE the
            // handler runs, terminate AFTER, so the in-flight event (e.g.
            // relaying this re-INVITE's response) is still serviced before
            // teardown.
