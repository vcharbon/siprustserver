...
use crate::store::{CallStore, InMemoryCallStore, PartitionRole, PutOpts};
...
/// Spawn a core on simulated SIP and replication fabrics with no peers: the
/// store is seeded by hand and nothing pulls it.
async fn node(ordinal: &str) -> Node {
