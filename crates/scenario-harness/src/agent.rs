...
/// send core (upstreamneed-034 ask C). Every primitive is implemented once,
...
impl Harness {
    ...
    /// upstreamneed-036 ask A its unread datagrams are already recorded at
...
// ---------------------------------------------------------------------------
// Transaction-layer receive view (upstreamneed-034)
// ---------------------------------------------------------------------------
...
///   see upstreamneed-033 D2), not something the harness may hide.
...
/// §17.1.1.3 UAS-side ACK obligations (upstreamneed-036 ask B) — the receive-side
...
pub struct Agent {
    ...
    /// §17.2 once-and-only-once receive view ([`TxnView`], upstreamneed-034).
    ...
    /// upstreamneed-036 ask B). Shared across clones — one table per logical UA.
...
impl Agent {
    ...
    /// replaces `quiesce`-as-choreography (upstreamneed-033 ask C): wait (up to
    ...
    ///
    /// NOTE (upstreamneed-034): the agent now has a §17.2 receive view
...
impl ClientInvite {
    ...
    /// SIPp-`optional` semantics for the load lane (upstreamneed-033 ask D):
...
impl<'a> InDialogRequest<'a> {
    ...
    /// `INFO`/other with a real payload: an `application/example-binary` SUP body,
...
/// exactly like [`ClientInvite`]'s finals (upstreamneed-034 ask B). Non-INVITE
...
/// RFC 3261 §17.1.1.3 client-transaction auto-ACK context (upstreamneed-034
...
mod auth_seam_tests {
    ...
    /// upstreamneed-033 ask D: `try_expect_final` absorbs (and collects) any
    ...
    /// upstreamneed-033 ask C: `try_receive_tolerating_blocking` waits for the
...
#[cfg(test)]
mod txn_view_tests {
    //! upstreamneed-034: the §17.2 once-and-only-once receive view ([`TxnView`]).
