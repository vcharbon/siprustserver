...
impl ContentLengthRule {
    ...
    ///
    /// The body may be **binary** (a non-UTF-8 in-dialog INFO/`example-binary`
...
/// perfectly-correct wires (upstreamneed/014); only a response whose top branch
...
mod tests {
    ...
    /// A faithful **non-UTF-8** body (a binary `application/example-binary` INFO
    /// payload) whose Content-Length is the correct RAW byte count must NOT be
    /// flagged. Regression guard for upstreamneed-023: `String::from_utf8_lossy`
    ...
    fn binary_body_with_correct_content_length_is_clean() {
        ...
        let mut raw = format!(
            "INFO sip:bob@127.0.0.1:5070 SIP/2.0\r\n\
             ...
             CSeq: 2 INFO\r\n\
             Max-Forwards: 70\r\n\
             Content-Type: application/example-binary\r\n\
             Content-Length: {}\r\n\r\n",
            body.len()
        )
        .into_bytes();
    ...
    // §17.1.3 branch-first correlation (upstreamneed/014): a relay forwarding
