...
fn label(msg: &SipMessage) -> String {
    match msg {
        SipMessage::Request(r) => format!("{} (CSeq {})", r.method.as_str(), r.cseq.seq),
        SipMessage::Response(r) => {
            format!("{} to {} (CSeq {})", r.status, r.cseq.method.as_str(), r.cseq.seq)
        }
    }
}
    ...
    F: FnMut(Transition<'_>),
{
    ...
    for ev in &slot.ordered {
        ...
        match f.msg {
            SipMessage::Request(req) if req.method.as_str() == "ACK" => {
                let key = RoundKey {
                    cseq: req.cseq.seq,
                ...
                match (rounds.get(&key), f.sdp.clone()) {
                    ...
                    (_, Some(body)) => {
                        if let Some((offer, offer_label)) = delayed.remove(&(req.cseq.seq, f.sent))
                        {
            ...
            SipMessage::Response(resp) => {
                ...
                match rounds.get_mut(&key) {
                    ...
                    None => {
                        if resp.status >= 200 && resp.cseq.method.as_str() == "INVITE" {
                            delayed
