...
//!    (upstreamneed-033 ask A): a hop-routed ACK the LB re-emits with the R-URI
...
// (upstreamneed-022): the same primitive backs the functional/e2e multi-callee
...
// view that used them (upstreamneed-022); the mux's own demux never called them
// directly.
...
mod tests {
    ...
    //    (upstreamneed-022); its unit tests moved with it. The mux's
