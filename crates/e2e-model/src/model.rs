...
impl Input {
    ...
    /// (paired with its `JsonSchema` via the shape's `params_schema` in
    /// `e2e-core`, so the schema the author saw IS the type `run` reads —
    /// `CallflowShape::params_schema`). `T` should carry
