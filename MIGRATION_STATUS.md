...
| **Network / UDP** (transport, SignalingNetwork) | `crates/sip-net` *(slice 2)* | `src/sip/{UdpTransport,SignalingNetwork,BufferedUdpEndpoint}.ts` | ✅ `SignalingNetwork` ported — trait (DI seam) + real (tokio `UdpSocket`) + simulated (in-memory fabric) + recording/`scopedAudit` & `paranoidInputs` contract decorators; 22 tests green. `UdpTransport` facade's Tier-1 brake (migration/11) + `UdpTransportMetrics` shape & registry wiring (migration/17) ported into `crates/b2bua`. **migration/23: `BufferedUdpEndpoint` was ported then removed (won't port).** The TS guard quarantined a blocking `getaddrinfo` inside Node's `send`; tokio has no such hazard (sends take an already-resolved `SocketAddr` — DNS is upstream + gated by `TargetAdmission`), per-peer queuing buys no isolation for real UDP, and the ported module was unwired and buggy, so `buffered.rs` was deleted. `ConnectivityGate` still **deferred** (Slice 2 §un-ported) |
...
- The four **Tier-1 overload-brake byte helpers** in `MessageHelpers.ts`
  ...
  `PreIngressHook` seam + simulated fabric).
  **AMENDED 2026-08-01 (tracker #4):** the byte-helper composition is gone — the
  brake parses above its threshold and classifies on the parsed request, so the
  templater + the emergency byte scan and their tests were deleted with it. The
  wired-hook cases (now including "a To-tagged re-INVITE is never braked", which
  the byte design could not express) remain in `b2bua/tests/tier1_brake.rs`.
...
- **`UdpTransport.ts`** — the **Tier-1 overload brake `preIngress` policy is now
  ...
  **migration/17: the `UdpTransportMetrics` *shape* + registry wiring is now
  ...
  sipjsserver @ `fffc4ac6`.
- **`BufferedUdpEndpoint.ts`** (non-blocking per-peer outbound drainer) —
  **removed (won't port).** It guarded against a blocking `getaddrinfo` inside
  Node's `send`, which has no analogue in tokio (Rust sends take an
  already-resolved `SocketAddr` — DNS happens once, upstream, and is gated by
  `TargetAdmission`), and per-peer queuing buys no isolation for real UDP
  (writability/backpressure is socket-wide). It was also unwired and carried
  several concurrency/lifecycle bugs. `migration/17`'s **counter value-shape**
  (`b2bua::metrics::BufferedSendCounters`, the six fields) is *kept* so the
  `UdpTransportMetrics` surface stays stable, but it now reads **permanently
  zero** — a later cleanup may drop it with the dashboard panels.
...
- **Tier-3 overload admission gate** — ✅ **ported in migration/09, but in the
  ...
  `buildStatelessReject503Buffer` were ported alongside the gate; since
  2026-08-01 (tracker #4) the reject is the one shared
  `b2bua::overload::build_reject_new_call_503`, fed by
  `sip_message::is_emergency_request`.
...
| `codec/call.proto` + `codec/call.proto.gen.cjs` (pbjs static module) | — | ❌ **removed (won't port)** — the protobuf codec was a JS/Redis-era experiment; the Rust port has no Redis (msgpack is the sole on-the-wire codec), so the schema + `protox`/`prost-build` codegen toolchain carried no value and was deleted (removed `proto/call.proto`, `build.rs`, `src/proto.rs`, the `prost`/`prost-build`/`protox` deps) |
| `codec/protobuf.ts` (`ProtobufLayer` + `toProtoObject`/`fromProtoObject` + `*IsNull`/`*Present`/`*Json` shims) | — | ❌ **removed (won't port)** — see the row above; `ProtobufCodec` + `src/codec/protobuf.rs` deleted. The `CallBodyCodec` trait remains the DI seam, but `MsgpackCodec` is the only impl |
...
| `round-trip-property.test.ts` `ProtobufLayer` cases | — | ❌ removed with the protobuf codec (was `tests/proto_codegen.rs` + `tests/codec_protobuf.rs`) — N/A, no protobuf codec |
...
- **Protobuf codec + `call.proto`** — ❌ **removed (won't port).** It was a
  JS/Redis-era experiment; the Rust port has no Redis and msgpack is the sole
  codec, so the schema, the `protox`/`prost-build` codegen toolchain, the
  `ProtobufCodec` impl, and their tests were deleted. The `CallBodyCodec` trait
  stays as the DI seam (one impl: `MsgpackCodec`).
...
self-overload gate is now a **real ELU/CPS gate** (`EluCpsGate`, migration/14 —
`AlwaysAdmitGate` remains the opt-out default); worker overload runs **both**
band classification **and** the per-worker AIMD rate-cap token bucket (now
ported in `load_observer.rs`).
...
- X3 cookie = HMAC-SHA256 truncated to 128 bits, base64url; HRW = SHA-1 as a
  non-crypto hash. X4 self-gate **now a real ELU/CPS gate** (`EluCpsGate`,
  migration/14; layered on OPTIONS health/band + `sip-net` tail-drop;
  `AlwaysAdmitGate` is the opt-out default). X5 lock-free registry (`ArcSwap`) +
  `broadcast` changes. X6 per-worker AIMD rate-cap token bucket **now ported**
  (`load_observer.rs`).
...
| `strategies/LoadBalancer.ts` | `strategies/load_balancer.rs` | ✅ HRW select + band filter + v=3 signed cookie (incl. the `e=<0\|1>` emergency flag + `above_critical`-band non-emergency bypass) + routing matrix; the per-worker AIMD rate-cap bucket is now ported in `load_observer.rs` and consumed via `try_consume_for` |
| `WorkerLoadObserver.ts` | `load_observer.rs` | ✅ ELU-band classification + hysteresis + `X-Overload` parse **plus the per-`(LB,worker)` AIMD token bucket** (`apply_aimd_step` additive-increase/multiplicative-decrease ladder + cooldown + `try_consume_for` consume path) |
...
| Source test | Rust home | Status |
|---|---|---|
| `load-balancer/{hmac-tampering,cookie-route-fallback,decode-forward-not-ready,decode-forward-respawn-window,unresolvable-id-falls-back,add-remove-resharding,initial-health,distribution,selectForNewDialog-overload(band+emergency-bypass **+ AIMD bucket**: `rate_cap_exhausted_when_winners_bucket_is_empty`, `unobserved_worker_is_admitted_without_a_bucket`)}` | `tests/load_balancer.rs` | ✅ 15 |
...
- ✅ **RESOLVED** — **AIMD per-worker rate-cap token bucket** (X6) is now ported in
  `load_observer.rs` (`WorkerState` bucket + `apply_aimd_step` increase/decrease/
  cooldown ladder + `try_consume_for`), so the LB now raises a rate-cap rejection
  on an empty bucket in addition to band classification + the CRITICAL filter.
  Pinned by the `load_observer::tests` AIMD-ladder/cooldown/consume unit cases +
  the `tests/load_balancer.rs` `selectForNewDialog-overload` bucket coverage.
- **Kubernetes registry** (`registry/kubernetes.ts`) + **k8s HMAC fs-watch** —
  production-only; static + simulated cover the slice.
- **Proxy failover / replication / call-limiter integration tests** (`failover/**`,
  k8s `proxy-*`) — these *proxy-side* tests depend on the unported B2BUA call cache +
  replication; only the proxy *request-path* `forwardBackup`/reverse-path mechanics
  are ported (covered by `decode-forward-*`). (The call-limiter *itself* is ported —
  see the Call-limiter row; this bullet is the proxy's limiter-coupled failover tests,
  not the limiter.)
...
- **`health-probe-late-reply`** — **now ported** as `tests/health_probe_late_reply.rs`
  (4 tests): a probe reply landing past the reap window is branch-correlated and
  keeps the worker alive (`late_replies_past_the_reap_window_keep_the_worker_alive`,
  `stable_alive_across_sustained_late_replies`), closing the late-reply endurance
  race. **`transparency/*` dual-mode** stays deferred — the happy-call, routing, and
  health-state transitions it asserts are covered by `transit_only`,
  `load_balancer_routing`, and `options_e2e`; the direct-vs-withProxy equivalence
  harness is not load-bearing for the proxy mechanics.
...
bucket; emergency bypasses the AIMD bucket) were picked up with the AIMD-bucket
item, not item 07, and **are now ported**: the LB raises
`SelectError::RateCapExhausted` when the winner's bucket is empty
(`load_balancer.rs:194` → `request.rs` 503 + Retry-After `worker_rate_capped`),
and emergency/in-dialog selects bypass the bucket. Covered by
`tests/load_balancer.rs::{rate_cap_exhausted_when_winners_bucket_is_empty,
unobserved_worker_is_admitted_without_a_bucket}`.
...
lands** (TODO(migration/08) in `overload.rs`). The per-worker AIMD rate-cap bucket
that keys on this ELU **is now ported** (X6, `load_observer.rs`), so the band/cap
ladder is **driven by this ~constant placeholder ELU** until a true busy-ratio ELU
lands — the AIMD item must not be assumed to react to real event-loop pressure yet.
Swap to `tokio::runtime::Handle::current().metrics()` busy-duration
...
| `CallLimiter*.ts` | `limiter.rs` (`CallLimiter` seam + `NoopLimiter`) + `limiter_http.rs` (`HttpCallLimiter` client) | ✅ b2bua-side seam + production HTTP client ported; the sliding-window limiter **server** is the standalone `call-limiter` crate (Call-limiter row). Default wiring still `NoopLimiter` (fail-open) unless `LIMITER_URL` is set |
| `b2bua/stack-identity.ts` | `stack_identity.rs` (+ `rules/relay.rs` leg builders) | ✅ Via/Contact `cr`/`lg` stamping + param codec; **emergency markers wired end-to-end** (slices 05+06, collapsed into one commit `72b864c`: the `;em=1`/`;emerg=1` marker primitives and their `legStackIdentity` wiring shipped together — branch `migration/05-…` carries **no standalone delta** and points at the slice-04 boundary `1b184b5`): the `;em=1` (Via) / `;emerg=1` (Contact) markers are now stamped on every real outbound hop by threading `is_emergency = call.emergency == Some(true)` (port of `legStackIdentity`, stack-identity.ts L137) through `relay::{leg_via,leg_contact,build_b_leg,ack_b_leg}` + `bye_on_dialog` to all ~20 `actions.rs`/`apply_route.rs` call sites — so an admitted emergency call's in-dialog traffic stays identifiable as emergency on the wire. **AMENDED 2026-08-01 (tracker #4):** the markers are a wire-visible signal with no in-tree reader — `buffer_has_emergency_marker` is deleted, and the Tier-1 brake admits ALL in-dialog traffic on the `To`-tag check, classifying emergency from the parsed `Resource-Priority` on initial INVITEs only. The `StackIdentity` public read-API (`from_config`/`advertised_{host,port}`) is a forward-looking consumer seam (no in-tree caller yet), mirroring the TS `StackIdentity` DI service |
...
| `options-keepalive-timeout.ts` | auto-cutoff: a leg that never answers its keepalive OPTIONS trips the per-leg `KeepaliveTimeout`, which ends the call and BYEs both legs (RFC 3261 §12.2.1.2); the silent leg's unanswered BYE times out and the call is reaped | `b2bua-harness/tests/keepalive_timeout.rs` | ✅ 1 |
| `keepalive-481.ts` | keepalive where bob answers the OPTIONS with `481 Call/Transaction Does Not Exist` → `handle-481` records the 481 CDR and `begin-termination` BYEs both legs (RFC 3261 §12.2.1.2); bob answers his BYE 481, which resolves the leg. Asserts one BYE to each peer on the recording | `b2bua-harness/tests/keepalive_481.rs` | ✅ 1 |
...
| `reinvite.ts` (`bobReInvite`) | in-dialog re-INVITE from the callee, offer in the re-INVITE: bob's re-INVITE(offer) relayed to alice → 200(answer) → ACK. Confirms the a-leg-target ACK the UAC core composes on receipt of the 200 (RFC 3261 §13.2.2.4) + response correlation in the from-a direction | `b2bua-harness/tests/reinvite.rs` | ✅ 1 |
...
- **Failover via `call_failure`** — **now ported**: `route-failure` / `no-answer`
  ...
  18x scenarios above.
- **Real CallLimiter** — **now ported** (Call-limiter row): the sliding-window
  `call-limiter` crate/server + the b2bua `HttpCallLimiter` client. This crate's
  default wiring is still `NoopLimiter` (fail-open) unless `LIMITER_URL` is set.
