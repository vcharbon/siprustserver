...
| **CallContext data model** | `crates/call` | `src/call/` (`CallModel`/`timer-helpers`/`codec`) | ✅ data model + codec ported — Call→Leg→Dialog structs + lens/index helpers + `callRef`/index-keys + positional-msgpack `CallBodyCodec`; 24 tests green. Stateful `CallState` now ported in `crates/b2bua` (in-memory; Redis/HA-replication transport still deferred). `TimerService` ported as the B2BUA-local `DelayQueue`. protobuf **schema + codegen toolchain** now ported (`proto/call.proto` + `build.rs` via `protox`/`prost-build`, 8 wire tests); the protobuf **codec impl** is still deferred ([slice below](#slice--callcontext-data-model-cratescall), [ADR-0008](docs/adr/0008-call-context-data-model.md)) |
...
| `codec/call.proto` + `codec/call.proto.gen.cjs` (pbjs static module) | `crates/call/proto/call.proto` + `build.rs` (codegen) + `src/proto.rs` (`proto::wire::*`) | ✅ **schema + codegen toolchain** — proto copied verbatim from source `49cf70f` (`src/call/codec/call.proto`; source tree HEAD `fffc4ac`); codegen via `protox` (pure-Rust .proto → FileDescriptorSet, no system `protoc`) + `prost-build::compile_fds` (Rust analogue of the `pbjs --target static-module` step; OUT_DIR artifact, not committed). The protobuf **codec impl** (the `model::Call` ↔ `proto::wire::Call` mapping with the `*IsNull`/`*Present`/`*Json` side-channels) is a separate stacked item. ([ADR-0011](docs/adr/0011-ha-replication-peer-to-peer.md) field-id rules) |
...
| `round-trip-property.test.ts` `ProtobufLayer` cases — wire-level slice (the full P1–P14 over real `Call` fixtures lands with the codec impl) | `tests/proto_codegen.rs` | ✅ 8 (wire P1 full-`Call` + per-leaf round-trip, encode non-empty/deterministic, `*IsNull`/`*Present` bits as independent wire fields, P7 byte sweep, `optional bytes` 3-state, retired-id 33/34/35/36 not-reused, `r#type`/`CSeq` codegen). Pure sync — no clock/timers, so the >60 s/fake-clock policy is N/A |
