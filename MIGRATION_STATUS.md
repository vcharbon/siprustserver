...
| **Network / UDP** (transport, SignalingNetwork) | `crates/sip-net` *(slice 2)* | `src/sip/{UdpTransport,SignalingNetwork,BufferedUdpEndpoint}.ts` | ✅ `SignalingNetwork` ported — trait (DI seam) + real (tokio `UdpSocket`) + simulated (in-memory fabric) + recording/`scopedAudit` & `paranoidInputs` contract decorators; 22 tests green. `UdpTransport` facade's Tier-1 brake (migration/11) + `UdpTransportMetrics` shape & registry wiring (migration/17) ported into `crates/b2bua`. **migration/23 (source SHA `fffc4ac6`): `BufferedUdpEndpoint` ported** (`buffered.rs`) — `BufferedUdpEndpoint::wrap` decorating any `UdpEndpoint` with one bounded drop-newest queue + drainer task per destination `SocketAddr` (the TS `peerKey(host,port)`), non-blocking `send_to` (pure enqueue, **no `.await`** so a synchronous burst can't let a drainer interleave), per-peer isolation (a hung inner `send_to` wedges only its own peer — the kindlab DNS-DoS head-of-line guard), `SendError` swallowed into `inner_send_errors`, idle-TTL sweep (`tokio::time::interval` on `sip-clock`'s monotonic-anchored `now_ms`, paused-clock-deterministic), `max_peers` idle-LRU eviction (`PeerEvictionStrategy` trait + `IdleLruStrategy`), `BufferedSendCounters`, and the `pending_work_delta` seam wired to `bump_in_flight` for the sim quiescence wait. 10 tests green (9 ported from `BufferedUdpEndpoint.test.ts` + 1 pinning the in-flight hook), all `#[tokio::test(start_paused)]` fake-clock. `ConnectivityGate` still **deferred** (Slice 2 §un-ported) |
...
| **CallContext data model** | `crates/call` | `src/call/` (`CallModel`/`timer-helpers`/`codec`) | ✅ data model + codec ported — Call→Leg→Dialog structs + lens/index helpers + `callRef`/index-keys + positional-msgpack `CallBodyCodec`; 24 tests green. Stateful `CallState` now ported in `crates/b2bua` (in-memory; Redis/HA-replication transport still deferred). `TimerService` ported as the B2BUA-local `DelayQueue`. protobuf **schema + codegen toolchain** (`proto/call.proto` + `build.rs` via `protox`/`prost-build`, 8 wire tests) **and the protobuf codec impl** (`ProtobufCodec` + the `model::Call` ↔ `proto::wire::Call` shims, 8 round-trip/property tests) now ported ([slice below](#slice--callcontext-data-model-cratescall), [ADR-0008](docs/adr/0008-call-context-data-model.md)) |
...
| `codec/call.proto` + `codec/call.proto.gen.cjs` (pbjs static module) | `crates/call/proto/call.proto` + `build.rs` (codegen) + `src/proto.rs` (`proto::wire::*`) | ✅ **schema + codegen toolchain** — proto copied verbatim from source `49cf70f` (`src/call/codec/call.proto`; source tree HEAD `fffc4ac`); codegen via `protox` (pure-Rust .proto → FileDescriptorSet, no system `protoc`) + `prost-build::compile_fds` (Rust analogue of the `pbjs --target static-module` step; OUT_DIR artifact, not committed). ([ADR-0011](docs/adr/0011-ha-replication-peer-to-peer.md) field-id rules) |
| `codec/protobuf.ts` (`ProtobufLayer` + `toProtoObject`/`fromProtoObject` + `*IsNull`/`*Present`/`*Json` shims) | `codec.rs` (`ProtobufCodec`) + `src/codec/protobuf.rs` (`model::Call` ↔ `proto::wire::Call` mapping) | ✅ **codec impl** — encodes the model against `proto/call.proto` via the shim. Shim is *smaller* than TS's: `Option<T>` / `Option<PolicyUpdateBody>` / `serde_json::Value` already encode the absent/null/empty + union distinctions the TS `*IsNull`/`*Present`/`*Json` flags fake under proto3 (the `*IsNull`/`*Present` wire bits the schema fixes are still driven, but from one typed field each). `featuresJson`/`extJson`/`policyUpdateHeadersJson`/`pendingInviteTxnJson` ride proto `string` as `serde_json` round-trips. `ActiveRule.params*` + `ruleState*` left at proto defaults (codec-unused in TS too). **Carry losses** (schema mirrors the older TS `Call`): `CallTopology.bak_gen` (the ADR-0014 `(p,b)` backup counter — TS topology is `{pri,bak,gen}`; reset to 0 on decode) + the ADR-0016 typed-ext slices (`sm_cursors`/`transfer`/`relay_first_18x`/`promote_pem`) have no proto field. Acceptable because msgpack is the replication codec + codec swap = fresh cluster (ADR-0011); if protobuf is ever promoted to replication, `call.proto` must first gain `bakGen` + the typed-ext fields |
...
| `round-trip-property.test.ts` `ProtobufLayer` cases — wire-level slice (schema/codegen) | `tests/proto_codegen.rs` | ✅ 8 (wire P1 full-`Call` + per-leaf round-trip, encode non-empty/deterministic, `*IsNull`/`*Present` bits as independent wire fields, P7 byte sweep, `optional bytes` 3-state, retired-id 33/34/35/36 not-reused, `r#type`/`CSeq` codegen). Pure sync — no clock/timers, so the >60 s/fake-clock policy is N/A |
| `round-trip-property.test.ts` `ProtobufLayer` cases — **codec impl** (P1/P1-P14/P7/P5 + `paranoidInputs` PA1/PA2 over real `Call` fixtures) | `tests/codec_protobuf.rs` | ✅ 8 (P1 representative full-value, P7 size sweep 0/1/1k/64k, P5 `callbackContext` None, null/presence distinctions incl. `policyUpdateBody` 3-state, PA2 empty→`Empty`, PA1 malformed/structurally-invalid→`Decode`, P1-P14 + P2/P3-determinism over a 200-case proptest pool from `arb_call` normalised to the proto int32/double domain). Pure sync — >60 s/fake-clock policy N/A. PA1 maps to malformed-**decode** (typed-`encode(&Call)` makes the TS malformed-encode unrepresentable, like P10/P11/P13) |
...
- **Protobuf codec + `call.proto`** — ✅ now ported (schema + codegen + the
  `ProtobufCodec` impl with the field-mapping shims). See the rows above; the
  remaining gap is the `parity` cross-codec decorator (next bullet).
